...
TEST_F(AuthServiceTest, Register_Success) {
  auto result =
      service_->Register("alice", "password123", "Alice", "alice@example.com");
...
TEST_F(AuthServiceTest, Register_WithAvatar) {
  auto result = service_->Register("alice", "password123", "Alice", "",
...
TEST_F(AuthServiceTest, UpdateProfile_Success) {
  ...
  auto result = service_->UpdateProfile(reg.user_id, "Alice Updated",
                                        "https://avatar.png", "Hello");
  EXPECT_TRUE(result.success);
...
TEST_F(AuthServiceTest, UpdateProfile_Partial) {
  auto reg =
      service_->Register("alice", "password123", "Alice", "", "old_avatar.png");
  ASSERT_TRUE(reg.success);
...
TEST_F(AuthServiceTest, FullFlow) {
  ...
  auto update =
      service_->UpdateProfile(reg.user_id, "Bob Updated", "", "Hi there");
