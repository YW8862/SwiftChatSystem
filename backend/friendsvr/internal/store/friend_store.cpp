...
std::string KeyFriendReqTo(const std::string& to_user_id, const std::string& request_id) {
    return std::string(K_FRIEND_REQ_TO) + to_user_id + ":" + request_id;
}
std::string KeyFriendReqFrom(const std::string& from_user_id, const std::string& request_id) {
    return std::string(K_FRIEND_REQ_FROM) + from_user_id + ":" + request_id;
}
std::string KeyFriendGroup(const std::string& user_id, const std::string& group_id) {
...
bool RocksDBFriendStore::RemoveFriend(const std::string& user_id, const std::string& friend_id) {
...
std::vector<FriendData> RocksDBFriendStore::GetFriends(const std::string& user_id,
...
bool RocksDBFriendStore::IsFriend(const std::string& user_id, const std::string& friend_id) {
    if (!impl_->db || user_id.empty() || friend_id.empty())
        return false;
    std::string value;
    return impl_->db->Get(rocksdb::ReadOptions(), KeyFriend(user_id, friend_id), &value).ok();
}

bool RocksDBFriendStore::UpdateRemark(const std::string& user_id, const std::string& friend_id,
                                      const std::string& remark) {
    if (!impl_->db || user_id.empty() || friend_id.empty())
        return false;

    std::string value;
    if (!impl_->db->Get(rocksdb::ReadOptions(), KeyFriend(user_id, friend_id), &value).ok())
        return false;
    ...
    rocksdb::WriteOptions wo;
    wo.sync = true;
    return impl_->db->Put(wo, KeyFriend(user_id, friend_id), SerializeFriend(d)).ok();
}

bool RocksDBFriendStore::MoveFriend(const std::string& user_id, const std::string& friend_id,
...
bool RocksDBFriendStore::CreateRequest(const FriendRequestData& req) {
    if (!impl_->db || req.request_id.empty() || req.from_user_id.empty() || req.to_user_id.empty())
        return false;
...
std::optional<FriendRequestData> RocksDBFriendStore::GetRequest(const std::string& request_id) {
    if (!impl_->db || request_id.empty())
        return std::nullopt;

    std::string value;
    if (!impl_->db->Get(rocksdb::ReadOptions(), KeyFriendReq(request_id), &value).ok())
        return std::nullopt;
...
bool RocksDBFriendStore::UpdateRequestStatus(const std::string& request_id, int status) {
    if (!impl_->db || request_id.empty())
        return false;
    ...
    rocksdb::WriteOptions wo;
    wo.sync = true;
    return impl_->db->Put(wo, KeyFriendReq(request_id), SerializeRequest(*req)).ok();
}

std::vector<FriendRequestData> RocksDBFriendStore::GetReceivedRequests(const std::string& user_id) {
...
std::vector<FriendRequestData> RocksDBFriendStore::GetSentRequests(const std::string& user_id) {
    std::vector<FriendRequestData> result;
    if (!impl_->db || user_id.empty())
        return result;
...
std::vector<FriendGroupData> RocksDBFriendStore::GetGroups(const std::string& user_id) {
...
bool RocksDBFriendStore::DeleteGroup(const std::string& user_id, const std::string& group_id) {
    if (!impl_->db || user_id.empty() || group_id.empty())
        return false;

    // 将该分组下所有好友的 group_id 置空
...
bool RocksDBFriendStore::Block(const std::string& user_id, const std::string& target_id) {
...
bool RocksDBFriendStore::Unblock(const std::string& user_id, const std::string& target_id) {
    if (!impl_->db || user_id.empty() || target_id.empty())
        return false;
...
bool RocksDBFriendStore::IsBlocked(const std::string& user_id, const std::string& target_id) {
    if (!impl_->db || user_id.empty() || target_id.empty())
        return false;

    std::string value;
    return impl_->db->Get(rocksdb::ReadOptions(), KeyBlock(user_id, target_id), &value).ok();
}

std::vector<std::string> RocksDBFriendStore::GetBlockList(const std::string& user_id) {
